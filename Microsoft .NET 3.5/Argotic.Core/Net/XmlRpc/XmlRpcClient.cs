...
namespace Argotic.Net
{
    ...
    public class XmlRpcClient
    {
        ...
        private static void AsyncSendCallback(IAsyncResult result)
        {
            ...
            if (result.IsCompleted)
            {
                ...
                if (client != null)
                {
                    ...
                    //------------------------------------------------------------
                    client.OnMessageSent(new XmlRpcMessageSentEventArgs(host, message, response, credentials, proxy, userToken));
        ...
        public XmlRpcResponse Send(XmlRpcMessage message)
        {
            ...
            //------------------------------------------------------------
            WebRequest webRequest   = XmlRpcClient.CreateWebRequest(this.Host, this.UserAgent, message, this.UseDefaultCredentials, this.Credentials, this.Proxy);
        ...
        public void SendAsync(XmlRpcMessage message, Object userToken)
        {
            ...
            //------------------------------------------------------------
            asyncHttpWebRequest = XmlRpcClient.CreateWebRequest(this.Host, this.UserAgent, message, this.UseDefaultCredentials, this.Credentials, this.Proxy);
        ...
        //	PRIVATE METHODS
        //============================================================
        #region CreateWebRequest(Uri host, string userAgent, XmlRpcMessage message, bool useDefaultCredentials, ICredentials credentials, IWebProxy proxy)
        /// <summary>
        ...
        ///     A <see cref="ICredentials"/> object that represents the authentication credentials provided by this client when making remote procedure calls. This parameter can be <b>null</b>.
        /// </param>
        /// <param name="proxy">
        ///     A <see cref="IWebProxy"/> object that represents the web proxy utilized by this client to proxy remote procedure calls. This parameter can be <b>null</b>.
        /// </param>
        /// <exception cref="ArgumentNullException">The <paramref name="host"/> is a null reference (Nothing in Visual Basic).</exception>
        /// <exception cref="ArgumentNullException">The <paramref name="userAgent"/> is a null reference (Nothing in Visual Basic).</exception>
        /// <exception cref="ArgumentNullException">The <paramref name="userAgent"/> is an empty string.</exception>
        /// <exception cref="ArgumentNullException">The <paramref name="message"/> is a null reference (Nothing in Visual Basic).</exception>
        private static WebRequest CreateWebRequest(Uri host, string userAgent, XmlRpcMessage message, bool useDefaultCredentials, ICredentials credentials, IWebProxy proxy)
        {
            //------------------------------------------------------------
            //	Local members
            //------------------------------------------------------------
