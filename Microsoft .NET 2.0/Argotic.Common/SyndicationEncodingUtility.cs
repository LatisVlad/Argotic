...
namespace Argotic.Common
{
    ...
    public static class SyndicationEncodingUtility
    {
        ...
        /// <exception cref="ArgumentNullException">The <paramref name="xml"/> is an empty string.</exception>
        ...
        public static XPathNavigator CreateSafeNavigator(Uri source, ICredentials credentials, IWebProxy proxy)
        {
            ...
            //------------------------------------------------------------
            return SyndicationEncodingUtility.CreateSafeNavigator(source, credentials, proxy, null);
        }
        #endregion
        ...
        public static XPathNavigator CreateSafeNavigator(Uri source, ICredentials credentials, IWebProxy proxy, Encoding encoding)
        {
            ...
            using (WebResponse response = SyndicationEncodingUtility.CreateWebResponse(source, credentials, proxy))
            {
                Stream stream                   = null;
        ...
        public static WebRequest CreateWebRequest(Uri source, ICredentials credentials, IWebProxy proxy)
        {
            ...
            if(source.IsAbsoluteUri)
            {
                ...
                else if (String.Compare(source.Scheme, Uri.UriSchemeHttps, StringComparison.OrdinalIgnoreCase) == 0)
                {
                    ...
                    if (credentials != null)
                    {
                        httpsRequest.Credentials    = credentials;
                    }
                    else
                    {
        ...
        public static WebResponse CreateWebResponse(Uri source, ICredentials credentials, IWebProxy proxy)
        {
            ...
            WebRequest webRequest   = SyndicationEncodingUtility.CreateWebRequest(source, credentials, proxy);
            if (webRequest != null)
            {
        ...
        public static string EncodeInvalidXmlHexadecimalCharacters(string content)
        {
            ...
            //	Local members
            //------------------------------------------------------------
            Regex invalidXmlUnicodeCharacters   = new Regex(@"#x((10?|[2-F])FFF[EF]|FDD[0-9A-F]|7F|8[0-46-9A-F]9[0-9A-F])");
        ...
        public static string RemoveInvalidXmlHexadecimalCharacters(string content)
        {
            ...
            //	Local members
            //------------------------------------------------------------
            Regex invalidXmlHexadecimalCharacters   = new Regex(@"#x((10?|[2-F])FFF[EF]|FDD[0-9A-F]|7F|8[0-46-9A-F]9[0-9A-F])");
